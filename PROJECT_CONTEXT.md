...
- Female pitch correction may still artifact if WSOLA `bestOffset` is too weak (energy-only, not full continuity correlation) — **verify on device**
...
1. **User test v1.6.5** on Lemur with female speakers — confirm whether WSOLA removed the grain/noise
2. If noise remains: upgrade `bestOffset` to real cross-correlation / normalized correlation against the previous overlap tail (true WSOLA), or fall back to pass-through when `|ratio-1|` is moderate
3. Then: improve mobile A/V sync beyond `playbackRate`
4. Later: professional UI; Native Android app
