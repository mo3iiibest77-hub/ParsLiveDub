// ParsLiveDub - Background Service Worker (v1.0)

let currentSession = null;
let offscreenReady = false;

chrome.runtime.onInstalled.addListener(() => {
  console.log('[ParsLiveDub] Installed / Updated');
});
...
async function ensureOffscreen() {
  ...
  await chrome.offscreen.createDocument({
    ...
    justification: 'Real-time audio capture and Persian live dubbing playback'
  });
  offscreenReady = true;
}
...
chrome.tabs.onRemoved.addListener((tabId) => {
  if (currentSession?.tabId === tabId) stopDubbing();
});
